...
await T('host+tools+toolsets', async () => {
    ...
    assert.ok(h.ccPlugins().some(p => p.manifest.name === 'demo') && h.ccPlugins().some(p => p.manifest.name === 'gm') && h.pi.skills.list().some(s => s.name === 'demo:hello') && h.pi.skills.list().some(s => s.name.startsWith('gm:')) && h.pi.agentExts.list().some(a => a.name === 'demo:rev'), 'cc-plugin load + gm node_modules discovery')
