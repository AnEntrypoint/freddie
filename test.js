...
await T('home+config+skin', async () => {
    const { getFreddieHome } = await import('./src/home.js'); assert.equal(getFreddieHome(), TEST_HOME)
    const { saveConfigValue, getConfigValue } = await import('./src/config.js')
    saveConfigValue('display.skin', 'ares'); assert.equal(getConfigValue('display.skin'), 'ares')
    const { setActiveSkin, getActiveSkin, listBuiltinSkins } = await import('./src/skin/engine.js')
    for (const n of ['default','ares','mono','slate']) assert.ok(listBuiltinSkins().includes(n))
    setActiveSkin('mono'); assert.equal(getActiveSkin().name, 'mono')
})
...
await T('tools+toolsets', async () => {
    ...
    assert.ok(names.length >= 50, 'tools count: ' + names.length)
    for (const n of ['bash','read','write','edit','grep','todo','memory','delegate','browser','approval','checkpoint','clarify','code_execution','cronjob','send_message','session_search','terminal','skill_manager','vision','tts','mixture_of_agents','osv_check','schema_sanitizer','mcp_tool','file_operations','patch_parser','tool_output_limits']) assert.ok(names.includes(n), 'missing: ' + n)
    assert.match(JSON.parse(await registry.dispatch('bash', { command: 'echo hi-freddie', timeout_ms: 5000 })).stdout, /hi-freddie/)
    const { getEnabledToolSchemas, _FREDDIE_CORE_TOOLS } = await import('./src/toolsets.js')
    assert.ok((await getEnabledToolSchemas(['core'])).length >= _FREDDIE_CORE_TOOLS.length)
})
