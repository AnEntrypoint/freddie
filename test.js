...
await T('host+tools+toolsets', async () => {
    ...
    assert.ok(h.ccPlugins().length === 1 && h.ccPlugins()[0].manifest.name === 'demo' && h.pi.skills.list().some(s => s.name === 'demo:hello') && h.pi.agentExts.list().some(a => a.name === 'demo:rev'), 'cc-plugin load')
    assert.equal((await h.hooks.invoke('preToolCall', { name: 'bash', args: {} }))?.behavior, 'block', 'cc PreToolUse deny routes')
