...
import { listSessions, listSessionsWithSummary, getMessages } from '../sessions.js'
...
export const HANDLERS = {
    ...
    resume: async (state, args) => {
        ...
        // silently picking the most recent session. listSessionsWithSummary
        // adds message_count + last_content per row (one query, no N+1) so
        // the picker can show "id, N msgs, updated_at, last message" per
        // line, the same one-line-per-row shape context-minimap.js's own
        // closed rows use.
        let target
        if (!wanted) {
            ...
            if (!state.ui) {
                // No TUI ui bag available (e.g. a non-interactive caller
                // reached this handler somehow) -- fall back to the old
                // most-recent behavior rather than hanging forever waiting
                // for a keypress no one can send.
                target = rows[0]
            } else {
                target = await new Promise((resolve) => {
                    state.pendingSessionPickResolve = resolve
                    state.ui.showSessionPicker(new SessionPicker(rows))
                })
                if (!target) return 'resume cancelled'
            }
        } else {
        ...
        // The "resumed X" marker is appended as a real system-role message
        // INSIDE state.messages, not returned for ui.note()'s ephemeral
        // notice trailer (context-minimap.js renders all notices below every
        // row, unconditionally — a resume marker landing there reads as
        // "stuck to the bottom" instead of threading at the point it
        // actually happened, i.e. right after the resumed history). As a
        // real row it inherits the minimap's normal turn-threading, weight
        // display, and scrollback position.
        state.messages = [
            ...msgs.map(m => ({ role: m.role, content: m.content, tool_calls: m.tool_calls || undefined, tool_call_id: m.tool_call_id || undefined })),
            { role: 'system', content: `resumed ${target.id.slice(0, 8)} (${msgs.length} messages) — ${target.title || '(untitled)'}` },
        ]
        return null
    },
    keys: async () => {
