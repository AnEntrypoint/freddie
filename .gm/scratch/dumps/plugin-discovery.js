        ...
        if (fs.existsSync(handlerFile)) {
            ...
            imports.push(
                import(pathToFileURL(handlerFile).href).then(handlerMod => {
                    // Multi-tool files export _tool0, _tool1, ... (documented
                    // convention, AGENTS.md "Adding a tool") alongside the
                    // single-tool _tool shape -- both must register, since a
                    // handler-only directory has no plugin.js to wire multiple
                    // tools in by hand.
                    const tools = [handlerMod._tool, ...Object.keys(handlerMod).filter(k => /^_tool\d+$/.test(k)).sort().map(k => handlerMod[k])].filter(Boolean)
                    ...
                    found.push({
                        ...
                        register({ pi }) { for (const t of tools) pi.tools.register(t) },
                    })
                })
            )
            continue
