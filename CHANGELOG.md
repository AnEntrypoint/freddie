...
### Fixed (libsql WAL warning)

- wasm32-wasi-vfs has no `xShmMap`, so `PRAGMA journal_mode=WAL` returns `rc=0` while leaving
  `journal_mode=delete` by construction. `gm/agentplug-libsql` (wasm-only) no longer attempts that
  doomed conversion. `libsql-plugkit-client` no longer dumps leftover
  `[libsql] WAL not in effect ... rc=0` diagnostics to stderr, which the host previously treated as
  an error for `/./sessions.db`.


### Removed (breaking, library consumers)
- `@earendil-works/pi-ai` dropped as a dependency (73 transitive packages removed). `src/agent/pi-bridge.js` deleted and `piCallLLM` removed from `src/index.js`'s public exports — freddie's real agent loop never used it (it routed through `acptoapi-bridge.js` already, per the documented resolver priority), and its only other consumer was the now-deleted file's own export. `acptoapiCallLLM` remains freddie's one LLM bridge, unchanged. `pi-tui` (a separate, independent package — confirmed zero shared dependencies with pi-ai) is unaffected.

### Added
- `run_flow` tool (`plugins/flow_skill`) walks a Mermaid/D2 flowchart in a SKILL.md from BEGIN to END. Junctions call the same LLM with path history, prior results, and remaining `{{blank}}` / `{{name:hint}}` markers; the LLM fills a blank only when that node is visited (`<blank name>` / `<choice>`). Task and decision nodes may emit `tool_calls` (host `dispatchTool`, nested `run_flow` banned). Direct tool calls remain legal for single steps. `/flow <name>` on TUI and REPL. Example: `skills/planning/demo-flow/SKILL.md`.
...
### Fixed (CLI hang)
- `plugins/core/core-cli/plugin.js` (`exec`/`run` actions) and `src/cli/print_mode.js` (`--print` path, shared teardown shape): `freddie exec`/`freddie run --print` hung indefinitely after printing a correct result instead of exiting, reproduced live and repeatably. Root-caused via three successive live diagnostics: acptoapi's own background subsystems (extra-providers probe, readiness's preemptive prober) hold undici HTTP sockets open past turn completion so `getGlobalDispatcher().close()` (which waits on in-flight requests) never settles; switching to `destroy()` cleared those sockets but the process still hung with zero active handles/requests per `process._getActiveHandles()`/`_getActiveRequests()`; adding the already-documented-but-never-wired-in `closeDb()` (libsql) + `closeAll()` (log streams) cleanup still left it hanging, pointing to a native-addon-level libuv reference outside JS introspection. Fix: `destroy()` the dispatcher + `closeDb()` + `closeAll()`, then an explicit `process.exit()` — safe now specifically because it only runs after every JS-owned handle is confirmed torn down, avoiding the original `UV_HANDLE_CLOSING` concern the code was written around (exiting while handles were still open mid-flight). Live-verified across `exec`, `exec --print`, and `run --print`, all exiting promptly (9-28s) instead of hanging past a 30-40s external timeout.

### Fixed (Windows tooling)
- `scripts/sync-upstream.mjs`'s `osv-scan-lockfile.mjs` invocation (`execFileSync(process.execPath, [osvScript], {shell:true})`) failed on Windows with `'C:\Program' is not recognized...` — `process.execPath` resolves to a path containing a space by default (`C:\Program Files\nodejs\node.exe`) and `shell:true` concatenates argv into one unquoted string for cmd.exe. Removed `shell:true` (a real binary invoked with an argv array needs no shell, unlike the adjacent legitimate `npm.cmd` invocation which was left unchanged). Live-verified against the real osv.dev API: 410 packages scanned, 0 vulnerabilities.

### Dependencies
- `gm-plugkit` `^2.0.2464` -> `^2.0.2496`, `gm-skill` `^2.0.2464` -> `^2.0.2496`, `plugsdk` `^1.0.21` -> `^1.0.26` via `scripts/sync-upstream.mjs`. Removed the dead `overrides.fast-xml-builder` entry from `package.json` — zero live consumer in the resolved dependency tree (`find node_modules -iname fast-xml-builder` returns nothing); it was the last real remnant among 36 open GitHub dependabot alerts, all others already fixed or absent from the tree, cross-checked package-by-package (`undici` 8.10.0, `postcss` 8.5.25, `js-yaml` 5.2.3+4.3.0, `body-parser` 2.3.0, `protobufjs` 7.6.5, `ws` 8.21.2, `qs` 6.15.3 all `>=` their fixed versions; `ip-address`/`form-data`/`brace-expansion`/`@mariozechner/pi-coding-agent` confirmed absent). `npm audit`: 0 vulnerabilities.
