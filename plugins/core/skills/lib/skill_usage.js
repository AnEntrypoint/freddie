...
export const skillUsageTool = ({
    ...
    handler: async ({ action, name, session_id = null, limit = 20 }) => {
        const d = await init()
        if (action === 'record') { await d.prepare(`INSERT INTO skill_usage (name, ts, session_id) VALUES (?, ?, ?)`).run(name, Date.now(), session_id); return { ok: true } }
