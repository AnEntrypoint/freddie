...
import { tgWithRetry, withProxy } from './network.js'
...
export class TelegramAdapter extends EventEmitter {
    ...
    async _loop() {
        while (this._running) {
            try {
                ...
                const data = await tgWithRetry(async () => {
                    const res = await fetch(url, withProxy({}, this.proxy))
                    return res.json()
                })
    ...
    async send(reply) {
        ...
        return tgWithRetry(() => fetch(url, withProxy({ method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ chat_id: reply.to, text: reply.text }) }, this.proxy)).then(r => r.json()))
    }
}
