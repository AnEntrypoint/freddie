...
export default {
    ...
    register({ gui }) {
        ...
        gui.route('POST', '/v1/chat/completions', async (req, res) => {
            ...
            try {
                // If this HTTP passthrough is the FIRST LLM dispatch in the
                // process (dashboard running, no agent turn yet), acptoapi's
                // readiness prober lazily starts on THIS sdk.chat() call --
                // src/agent/llm_provider_warmup.js's model_preference-scoped
                // ACPTOAPI_READINESS_TOPK cap only ever gets set inside
                // resolveCallLLM()'s warmExtraProviders() call, which this
                // route never reaches. warmExtraProviders() is idempotent
                // (single memoized promise) and safe to call from any entry
                // point, so calling it here closes that race for this route
                // too, same as every agent-loop call site already does.
                await warmExtraProviders()
