...
async function init() {
    const d = await db()
    await d.exec(`CREATE TABLE IF NOT EXISTS file_state (id INTEGER PRIMARY KEY AUTOINCREMENT, session_id TEXT, file_path TEXT NOT NULL, action TEXT NOT NULL, ts INTEGER NOT NULL)`)
    return d
}

export const fileStateTool = ({
    name: 'file_state',
    toolset: 'core',
    ...
    handler: async ({ action, session_id, file_path, op }) => {
        const d = await init()
        if (action === 'record') { await d.prepare('INSERT INTO file_state (session_id, file_path, action, ts) VALUES (?, ?, ?, ?)').run(session_id, file_path, op, Date.now()); return { ok: true } }
