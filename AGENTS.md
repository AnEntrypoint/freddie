...
- `anentrypoint-design` — webjsx + ripple-ui. **All GUI for freddie and thebird lives here.** Source lives in its own repo (`AnEntrypoint/design`), vendored as a git submodule at `./design` in this repo for easy editing; freddie's runtime code still consumes it from the npm registry at the `latest` dist-tag (never a pinned range) — the submodule does not change any import path. For local SDK iteration, edit `./design`, rebuild via `node scripts/build.mjs` inside it, then `npm publish` from there to flow the change to the `latest` dist-tag everyone consumes. Do NOT add React.
- `acptoapi` — THE LLM SDK (see "acptoapi is THE SDK" below). Source lives in its own repo (`AnEntrypoint/acptoapi`), vendored as a git submodule at `./acptoapi` in this repo for easy editing; freddie's `package.json` still pins it from the npm registry with a caret range (see Versioning below) — the submodule does not change that. Edit `./acptoapi`, then `npm publish` from there to flow the change downstream.
...
- Freddie installs plugsdk from registry, NOT via `file:` dep. (A historical `"link": true` lockfile entry that used to block `npm ci` does not reproduce in the current lockfile — confirmed clean.) The `./plugsdk` submodule does not change this — it's for editing the source and pushing/publishing, not a local dependency link.
