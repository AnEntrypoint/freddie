...
- **Bulk-rename: git grep is case-sensitive on literal patterns** — `git grep -lI foph` only matches lowercase. To find every case variant during a rename refactor, use `git grep -liI -e <lower> -e <Title> -e <UPPER>` (per-pattern `-i` requires `-e` form). Single-form check is a false-clean trap. (Confirmed 2026-05-03 during foph→freddie rename: pass 1 reported "0 remaining" while `Foph` camelCase imports and `FOPH_*` env constants survived and broke `getFophHome` import.)
- **src/home.js legacy fallback design** — `getFreddieHome()` reads `$FOPH_HOME` / `$FOPH_PROFILE` env vars and `~/.foph` directory as fallback when `~/.freddie` does not exist, so existing users keep their config/sessions/skills/auth across the rename. The legacy strings are written as concatenated literals (`'FOPH' + '_HOME'`, `'.' + 'foph'`) specifically so a future bulk regex pass cannot clobber them. Do not "clean up" by inlining the literals.
