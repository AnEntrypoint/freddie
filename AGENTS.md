...
- `@earendil-works/pi-telemetry` — a transitive dependency of `pi-ai` (not directly imported anywhere in freddie's own `src/`/`plugins/`), providing vendor-neutral telemetry contracts + typed schema utilities (root export + a `./testing` subpath). Freddie's own structured logging (`src/observability/log.js`) is a separate, already-correct concern — not a candidate for switching to pi-telemetry, noted here only so its presence in `node_modules` isn't mistaken for an unused/orphaned freddie dependency.
