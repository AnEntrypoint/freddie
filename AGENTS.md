...
Build: `node scripts/build.mjs` in the `anentrypoint-design` repo emits `dist/247420.js` + `dist/247420.css`, committed straight to `main` by `.github/workflows/publish.yml` on every push (no npm publish step — `dist/` is tracked in git specifically so a GitHub-raw CDN can serve it). `src/web/index.html` loads the SDK live from `https://raw.githack.com/AnEntrypoint/design/main/dist/247420.{js,css}`, NOT jsDelivr's `cdn.jsdelivr.net/gh/...` — `src/web/server.js`'s SPA-fallback comment explains why: jsDelivr's `gh/` mode caches an `@main` branch reference for up to 12h regardless of purge, while raw.githack.com's dev CDN serves the newest commit on `main` immediately, which the dashboard needs since it has no local npm install of the SDK to fall back on (no local `/vendor/` static-serve route, no offline SW precache of the SDK — `sw.js` only precaches shell HTML/app.js). To witness a local SDK edit before it lands on main, rebuild then temporarily point index.html's importmap/link at a local dev server serving your `dist/` output. Node-side imports (`state.js`, `routes.js`, `app.js`'s SSR/page-html helpers) resolve `anentrypoint-design` through `node_modules` from the `github:AnEntrypoint/design` dependency spec in package.json (no ref suffix = default branch HEAD at install time, never an npm registry version).
