...
Build: `node scripts/build.mjs` in the `anentrypoint-design` repo emits `dist/247420.js` + `dist/247420.css`, committed straight to `main` by `.github/workflows/publish.yml` on every push (no npm publish step — `dist/` is tracked in git specifically so a GitHub-raw CDN can serve it). `src/web/index.html` ships pointing the SDK at jsDelivr's `cdn.jsdelivr.net/gh/AnEntrypoint/design@main/dist/247420.{js,css}` (the same CDN mode the rest of the fleet uses, see "Kit consumption strategy" below) as a static fallback, but `src/web/server.js::createDashboard()` rewrites that `@main` to `@<sha>` on every `index.html`/SPA-fallback response, where `<sha>` is `git ls-tree HEAD design`'s current commit (`designSdkSha()`) — i.e. whatever the `design` submodule pointer is checked out to in this freddie checkout. Pinning to a specific commit sidesteps jsDelivr's `gh/@main` 12h branch-reference cache entirely (a specific commit's content is immutable and cached forever), while still tracking upstream automatically: `scripts/sync-upstream.mjs`'s daily `git submodule update --remote --checkout` (see Versioning below) advances the submodule pointer, and the next dashboard request picks up the new SHA with no separate bump step. Falls back to the static `@main` URL (rewrite skipped) only if `git ls-tree` can't run (e.g. a shallow clone with the submodule not checked out) — accepted as a rare degrade-to-slower-cache path, not a steady state. History: this was `raw.githack.com` (serves the newest `main` commit immediately, no cache lag) through two prior fix commits, then briefly a same-origin `/sdk/` route serving `design/dist`/`node_modules/anentrypoint-design/dist` directly (reverted — it silently froze every dashboard install at whatever `npm install` last resolved, never picking up new `design` pushes, defeating the live-tracking goal entirely); the SHA-pinned-jsDelivr scheme here was chosen to unify freddie onto the same CDN as the rest of the fleet while keeping both the immutability raw.githack lacked and the auto-tracking `/sdk/` lacked. Node-side imports (`state.js`, `routes.js`, `app.js`'s SSR/page-html helpers) resolve `anentrypoint-design` through `node_modules` from the `github:AnEntrypoint/design` dependency spec in package.json (no ref suffix = default branch HEAD at install time, never an npm registry version).
