...
| Package | Role |
|---|---|
| `@freddie/freddie-lsp` (this) | Service Definition: the service, provider registry keyed by branded id + extension mapping, per-query selection, request/result vocabulary, the `LspError` taxonomy |
| `@freddie/freddie-lsp-stdio` | Service Provider: a generic local backend that registers configured stdio language-server providers |
| `@freddie/freddie-tool-lsp` | Consumer: the model-facing `lsp` tool over `ctx.lsp` |
...
Providers register **capabilities**, not tools. `freddie-tool-lsp` is the only owner of the model-facing name, description, prompt guidance, schema, and presentation.
...
Indirectly, through `freddie-tool-lsp`, which owns the model-facing `lsp` schema, prompt, and rendered results while this registry contributes no prompt or schema itself.

#### KV Cache effect

No direct invalidation; `freddie-tool-lsp` owns request-prefix changes.
