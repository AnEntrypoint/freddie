...
| `persona` | — | Deployment persona template for `freddie-system-prompt`. |
| `toolOrder` | lexicographic | Explicit model-facing tool order for `freddie-system-prompt`. |
| `tools` | `{ mode: 'native' }` | Native, Code Mode, or combined model tool transport. |
| `freddieHome` | `$FREDDIE_HOME` or `~/.freddie` | Harness home shared by bash and local skill discovery. |
