...
Public request, value, version, and failure types are exported from the package root and `@freddie/freddie-message-feedback/types`; [`src/types.ts`](src/types.ts) is their source.
...
```yaml
- id: message-feedback
  name: '@freddie/freddie-message-feedback'
  config:
    maxNoteBytes: 8192
```
