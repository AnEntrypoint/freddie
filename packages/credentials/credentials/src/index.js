...
/** Both halves of a credential key; the `/` between them is what keeps it out of {@link REF_PATTERN}. */
...
/**
 * Brand a scope and an id as a credential key.
...
/**
 * Brand a stored `<scope>/<id>` string as a credential key. This is the
 ...
 *
 * A credential reference answers "what is behind this environment-variable
 ...
 *
 * A credential key answers "what credential does this plugin hold for
 ...
 * and {@link CredentialProvider#modifyRecord} is the only write path because a correct write
...
export class CredentialProvider extends Service {
   ...
   * rejects an empty value (use {@link CredentialProvider#unset}).
   ...
   * the reference, like {@link CredentialProvider#set}.
   ...
   * grant record payload is not interpreted on the way out.
   ...
   * {@link CredentialProvider#notifyUpdated} documents, for the record half of the seam.
  ...
  /** The contained dispatch both notifications run through; see {@link CredentialProvider#notifyUpdated}. */
