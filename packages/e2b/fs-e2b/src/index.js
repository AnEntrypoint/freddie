...
const VERSION_METADATA_KEY = 'freddie-version'
...
function decodeText(bytes, displayPath, binarySampleBytes) {
...
export class E2BFileSystem extends FileSystem {
  ...
  async canonicalPath(sandbox, path, signal) {
    try {
  ...
  async writeAtomic(target, content, existing, createIfAbsent, signal) {
    ...
    const stagingDirectory = posix.join(posix.dirname(targetPath), `.freddie-${randomUUID()}.tmp`)
