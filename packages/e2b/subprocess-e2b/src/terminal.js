...
const TERMINAL_RUNNER_SOURCE = [
  ...
  'set -euo pipefail',
  'freddie_state=$1',
  'mapfile -d \'\' -t freddie_env < "$freddie_state/environment"',
  'mapfile -d \'\' -t freddie_argv < "$freddie_state/argv"',
  'freddie_output_marker=$(<"$freddie_state/output-marker")',
  'rm -f -- "$freddie_state/environment" "$freddie_state/argv" "$freddie_state/output-marker" "$freddie_state/runner.bash"',
  'if (( ${#freddie_argv[@]} == 0 )); then',
  "  printf 'terminal runner received empty argv\\n' >&2",
  '  exit 125',
  'fi',
  'printf \'%s\' "$freddie_output_marker"',
  'exec env -i -- "${freddie_env[@]}" "${freddie_argv[@]}"',
  '',
].join('\n')
...
export async function spawnE2BTerminal(runtime, spec, stateDir, pollMs) {
  ...
  const outputMarker = Buffer.from(`freddie-e2b-bootstrap:${randomUUID()}`)
