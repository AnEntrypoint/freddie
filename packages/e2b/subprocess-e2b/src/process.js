...
function commandText(spec, paths) {
  const encoder = `"$freddie_e2b_env_bin" -i "$freddie_e2b_node" -e ${quoteE2BShellArg(OUTPUT_ENCODER_SOURCE)}`
  const stdoutRedirect = hasSpill(spec.stdio.stdout)
    ? `> >("$freddie_e2b_tee" --output-error=warn-nopipe >("$freddie_e2b_head" -c ${spec.stdio.stdout.spill.maxBytes} > ${quoteE2BShellArg(paths.stdout)}) | ${encoder} 2>/dev/null)`
    : `> >(${encoder} 2>/dev/null)`
  const stderrRedirect = hasSpill(spec.stdio.stderr)
    ? `2> >("$freddie_e2b_tee" --output-error=warn-nopipe >("$freddie_e2b_head" -c ${spec.stdio.stderr.spill.maxBytes} > ${quoteE2BShellArg(paths.stderr)}) | ${encoder} >&2 2>/dev/null)`
    : `2> >(${encoder} >&2 2>/dev/null)`
  const inner = [
    'set +e',
    'freddie_e2b_env_bin=$1',
    'freddie_e2b_node=$2',
    'freddie_e2b_ps=$3',
    'freddie_e2b_tr=$4',
    'freddie_e2b_tee=$5',
    ...
    'freddie_e2b_rm=$7',
    'shift 7',
    'freddie_e2b_pgid="$("$freddie_e2b_ps" -o pgid= -p "$$" | "$freddie_e2b_tr" -d " ")"',
    `printf '%s\\n' "$freddie_e2b_pgid" > ${quoteE2BShellArg(paths.pid)}`,
    `mapfile -d '' -t freddie_e2b_env < ${quoteE2BShellArg(paths.environment)}`,
    `"$freddie_e2b_rm" -f -- ${quoteE2BShellArg(paths.environment)}`,
    `"$freddie_e2b_env_bin" -i -- "\${freddie_e2b_env[@]}" "$@" ${stdoutRedirect} ${stderrRedirect}`.trimEnd(),
    'freddie_e2b_status=$?',
    `printf '%s\\n' "$freddie_e2b_status" > ${quoteE2BShellArg(paths.status)}`,
    'wait',
    'exit "$freddie_e2b_status"',
  ].join('\n')
  const argv = spec.argv.map(quoteE2BShellArg).join(' ')
  const bootstrap = [
    `mapfile -d '' -t freddie_e2b_env < ${quoteE2BShellArg(paths.environment)}`,
    'freddie_e2b_env_bin="$(command -v env)"',
    'freddie_e2b_setsid="$(command -v setsid)"',
    'freddie_e2b_bash="$(command -v bash)"',
    'freddie_e2b_node="$(command -v node)"',
    'freddie_e2b_ps="$(command -v ps)"',
    'freddie_e2b_tr="$(command -v tr)"',
    'freddie_e2b_tee="$(command -v tee)"',
    'freddie_e2b_head="$(command -v head)"',
    'freddie_e2b_rm="$(command -v rm)"',
    'for freddie_e2b_tool in "$freddie_e2b_env_bin" "$freddie_e2b_setsid" "$freddie_e2b_bash" "$freddie_e2b_node" "$freddie_e2b_ps" "$freddie_e2b_tr" "$freddie_e2b_tee" "$freddie_e2b_head" "$freddie_e2b_rm"; do',
    '  [[ "$freddie_e2b_tool" == /* && -x "$freddie_e2b_tool" ]] || exit 125',
    'done',
    `exec "$freddie_e2b_env_bin" -i -- "\${freddie_e2b_env[@]}" "$freddie_e2b_setsid" --wait -- "$freddie_e2b_bash" -c ${quoteE2BShellArg(inner)} freddie-e2b "$freddie_e2b_env_bin" "$freddie_e2b_node" "$freddie_e2b_ps" "$freddie_e2b_tr" "$freddie_e2b_tee" "$freddie_e2b_head" "$freddie_e2b_rm" ${argv}`,
  ].join('\n')
  return bootstrap
}
