 ...
 *
 * `freddie-system-prompt` owns the global persona as its own config, and registers
 ...
 * exactly like the per-child persona `freddie-subagent` installs; mounted globally
 ...
 * - `text`: Persona prose rendered as the `deployment:persona` section. A
 *   template: complete `{{…}}` groups interpolate strictly against registered
 *   prompt variables. Empty text drops the section at render, matching the
 *   registry.
 * - `complete`: Make this persona the complete system prompt, suppressing
 *   every other section.
