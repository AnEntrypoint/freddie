...
Context for a grounded [Dream-RSI](https://www.dream-rsi.com/) replay selection. It adds a replay directive only after the same Session records a successful `gm_dream_replay` tool result whose candidate ranking includes the deployed baseline and whose selected score is not lower than that baseline.
