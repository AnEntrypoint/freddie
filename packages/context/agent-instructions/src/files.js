...
import { freddieHomeDisplay } from '@freddie/freddie-home-paths'
...
async function discoverInstructionFiles(
  ...
  const userGlobal = join(config.freddieHome, USER_GLOBAL_FILE)
  ...
  switch (userGlobalProbe.kind) {
    case 'present':
      addFile({
        ...
        displayPath: userGlobalDisplayPath(config.freddieHome),
...
export async function probeScopeInstruction(
  ...
  const file = {
    absolutePath,
    displayPath: directory === USER_GLOBAL_DIRECTORY ? userGlobalDisplayPath(resolved.freddieHome) : relativeDisplay(projectRoot, absolutePath),
    target,
...
function userGlobalDisplayPath(freddieHome) {
  return `${freddieHomeDisplay(freddieHome)}/AGENTS.md`
}
