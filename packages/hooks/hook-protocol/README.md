...
The **shared core** of the Claude Code / Codex hook wire protocol. NOT a cordis plugin — it registers nothing and injects nothing. It is a **library** of dialect-neutral primitives the two bridge plugins (`@freddie/freddie-hooks-claude-code`, `@freddie/freddie-hooks-codex`) import so neither re-implements the identical halves of the protocol.
...
| Concern | Here (`freddie-hook-protocol`) | The bridge (`freddie-hooks-claude-code` / `-codex`) |
|---|---|---|
...
Indirectly, through `freddie-hooks-claude-code` and `freddie-hooks-codex`, which can turn parsed hook output into prompt context, blocked outcomes, or continuation feedback.
