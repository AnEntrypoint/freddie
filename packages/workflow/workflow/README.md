...
`@freddie/freddie-workflow-worker-thread` is the current engine and `@freddie/freddie-tool-workflow` is the model-facing consumer. A future process or sandbox engine can replace the implementation without changing the tool.

The package root is the Host face. The browser-safe `@freddie/freddie-workflow/types` subpath contains run identities, metadata, results, and observe-only lifecycle payloads without importing `Agent`, Cordis services, or Host context declarations; Host-only `WorkflowStartRequest` and `WorkflowRun` live behind the package root.
...
Indirectly, through `freddie-tool-workflow` and a workflow engine, which create child-agent requests and return a retained parent tool result.
...
- **Background ownership is consumer-mediated** — `tool-workflow` can transfer a run to the owner-scoped job registry, while direct engine consumers still own and await their live handles.
