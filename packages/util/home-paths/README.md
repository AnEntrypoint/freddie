...
`freddieHomeDisplay()` names an active root symbolically for user-facing paths: `~/.freddie` for the default home, `$FREDDIE_HOME` for any configured home. It never leaks an absolute machine path.

`FREDDIE_HOME_DIR_NAME` owns the default user-data directory name: `.freddie`.

`defaultDshHome()` returns the default Freddie home by joining the operating-system home directory with `.freddie`, using Node's platform path rules.
