# freddie-atomic-write

Zero-dependency atomic file replacement shared by file-backed stores that must never leave partial, symlink-hijacked, or wider-than-intended content on disk — the user-settings document (`freddie-settings-file`) and the credentials store (`freddie-credentials-local`).
