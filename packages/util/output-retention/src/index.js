 ...
 * (head strategy). `push()` reports, per unit, whether it
...
export class ItemRetainer {
   ...
   * @returns The per-push PushDecision.
   */
  push(item) {
    this.seen++
   ...
   *
   * @returns The RetainedItems snapshot (safe to group/sort downstream).
   */
  finish() {
 ...
 * (head/tail/headTail strategy). All three strategies share one prefix/suffix
...
export class TextRetainer {
  ...
  /** @param strategy One TextRetentionStrategy variant; byte budgets must be non-negative integers. */
  constructor(strategy) {
   ...
   *
   * @returns The RetainedText snapshot (safe to hand to a formatter).
   */
  finish() {
...
/**
 * Turn a RetentionNotice into a one-line footer: the library-owned
