...
A package brands the ids it owns — `CallId` in `freddie-llm`, the shared agent/session `SessionId` in `freddie-session`, and `JobId` in `freddie-jobs`. Brand cross-package ids that could plausibly be confused; not every string needs one.

This package owns only the primitive. Keeping it dependency-free lets `freddie-jobs`, for example, brand `JobId` without importing an unrelated capability package merely to reach `Branded`.
