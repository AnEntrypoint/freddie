 ...
 * `@freddie/freddie-fs` Service Definition. It extends `LocalFileSystem` so all
 ...
 * stays `ctx.shell`'s job (`@freddie/freddie-bash-sandbox`). This mirrors the
 ...
 * escalation retry lives in the tool layer (`@freddie/freddie-tool-fs`),
 * exactly as bash's does.
 *
 * @module @freddie/freddie-fs-sandbox
 */
 ...
 * INSTEAD OF `freddie-fs-local`, together with a `ctx.sandboxPolicy`, is the whole
 * swap — the model-facing tools are untouched). Its configured default mode is
 * the capability fact exposed by {@link sandboxMode}; `freddie-tool-fs` resolves
