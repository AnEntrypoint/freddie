...
const PROJECT_FREDDIE_RANK = 100
const PROJECT_AGENTS_RANK = 200
const CUSTOM_RANK = 300
const USER_FREDDIE_RANK = 400
...
export class FileSystemSkillProvider {
  ...
  async roots(cwd) {
    ...
    if (this.includeDefaultRoots && cwd !== undefined) {
      ...
      roots.push(
        { path: join(projectRoot, '.freddie/skills'), source: 'project-freddie', rank: PROJECT_FREDDIE_RANK, projectRoot },
    ...
    if (this.includeDefaultRoots) {
      roots.push(
        { path: join(this.freddieHome, 'skills'), source: 'user-freddie', rank: USER_FREDDIE_RANK, skipSystem: true },
...
class SkillWatchManager {
  ...
  async handleAncestorWatchEvent(
    state,
    mode,
  ) {
    let current
