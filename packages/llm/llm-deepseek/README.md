...
This package owns the `deepseek-official` provider route; registering another adapter for `deepseek-official` itself throws `LlmError('DUPLICATE_ADAPTER')`.
...
DeepSeek request identity is separate from app attribution. After credential resolution, every provider request carries `x-freddie-user-id` with the stable anonymous id from [`@freddie/freddie-anonymous-user-id`](../../identity/anonymous-user-id/README.md); a request carrying `GenerateOptions.sessionId` also sends that exact value as `x-freddie-session-id`, while a direct call without a session omits the session header. Both headers go to the resolved `baseURL`, including a configured gateway, and remain outside the request body and model-visible content.
