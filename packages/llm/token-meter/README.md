...
```yaml
- name: '@freddie/freddie-token-meter'
- name: '@freddie/freddie-compaction-basic'
```

Both plugins have usable defaults. The meter remains independent of model routing and optional compaction. A deployment configures capacity on its LLM adapter and compaction policy on `freddie-compaction-basic`.

## Model Experience

Indirectly, through consumers such as `freddie-compaction-basic`; the service itself adds no prompt, message, schema, tool, or model call.
