...
- `ensureDaemon`'s liveness check adds a `process.kill(pid, 0)` probe on top of `.status.json`'s `ts` freshness, closing a real gap found live this session: a daemon killed moments ago still reads "fresh" against a five-minute staleness window if only `ts` is checked, since the status file isn't rewritten on process death. A pid that is still alive but whose `ts` has not moved for four minutes (just under the five-minute dead-status window) is hung: `dispatch()` throws immediately instead of waiting the remaining timeout. A healthy runner can go more than a minute between `.status.json` heartbeats; sixty seconds was a false positive against that cadence.
