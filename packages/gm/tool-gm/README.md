...
| `gm_dream_discovery_record` | `dream-discovery-record` | `id`, `evaluator_receipt_id` | Record one ledger-bound discovery from an evaluator receipt. |
| `gm_dream_world_seal` | `dream-world-seal` | `world_id`, `discovery_ids` | Seal a replay world from recorded discovery IDs. |
| `gm_dream_replay` | `dream-replay` | `baseline_policy_id`, `policy_ids`, `world_ids` | Compare registered policies over sealed worlds without executing them. |
...
- Dream-RSI metrics are derived by GM from completed session-bound dispatch evidence: target comes from the recorded verb/fingerprint, score from its exit code, and cost from the recorded operation count. GM re-derives these values from the ledger when it consumes a receipt, so no key or external evaluator provider is required. A domain-specific evaluator may later replace this metric without widening model authority.
