...
Client module system: the browser peer of Node's internal ESM loader, built as a lazy CJS table. The web shell mounts the framework cordis Loader for entry governance (fiber lifecycle, inject waiting, update/refresh) and injects this package's `ClientModuleLoader` through its `internal` contract — the Loader side's only consumption point is `EntryTree.import`, so replacing `internal` replaces exactly "how plugin code arrives" and nothing else.
...
Resolution branch order (`import(specifier)`): platform seed word → shell instance; memoized record → exports; graph row (`window.__FREDDIE_BOOT__`) → register its classic-script factory; registered factory → materialize; anything else throws — the runtime mirror of the build-time bundle purity gate. The synchronous `require` handed to factories walks the same order minus the asynchronous graph-row load and records observed edges into the module record. `prefetch` is the stage-one arrival hook (script load and factory registration only; concurrent calls share one in-flight task); `invalidate` drops a non-bootstrap factory and materialized record so the next prefetch/import reloads the script (the HMR hook).

The Node half scans enabled Loader entries for web `freddie.client` packages, resolves each `exports["./client"]`, hashes the package's whole `src/` tree into the row's rev, carries package-specific `freddie.client.external` requests, orders dynamic providers before consumers, and serves every file of that tree under `/plugins/<id>/~<rev>/<path>` (HEAD returns the GET headers and no entity body). The row's URL is the entry's real path under that prefix, so every sibling import a bundle makes resolves under the same rev segment: a request naming the row's current rev answers `cache-control: public, max-age=31536000, immutable`, any other or absent rev segment answers `no-cache`, and every response carries a strong size+mtime `ETag` honoured as a `304`. There is no `client.js` alias and no redirect. The boot injections carry one `modulepreload` hint per script under the entry directory of each `immediately` row (measured to win at an emulated 40ms RTT; hinting every row measurably loses), and the import map points a workspace wire-layer specifier at its real served path rather than at the `/workspace` route's redirect. Missing entry directories share one build instruction followed by a package/path list, while unrelated filesystem errors remain separate failures.

`freddie.client.external` is an optional exact-specifier request list beyond the implicit baseline: shell-seeded React, Cordis, and static UI libraries plus parser-preloaded runtime. A request is answered by the dynamic package row it names or an exact static-table key; only a trailing `/client` aliases a package row, and there is no provider-alias declaration. Type-only imports are erased and create no request. Composition rejects malformed requests, missing suppliers, self-requests, and synchronous request cycles; import and prefetch recursively register dynamic suppliers before their consumers materialize. See [shared modules and the module graph](../AGENTS.md#shared-modules-and-the-module-graph).
...
- **No unload bookkeeping of its own** — style removal and fiber teardown ordering live with the HMR driver (`@freddie/freddie-client-hmr`); the loader only inventories owned style tag ids per record.
