...
export function ReferenceIcon({ kind, size = 16, className }) {
  switch (kind) {
    case 'session':
      return (
        h('svg', { width: String(size), height: String(size), class: className, viewBox: '0 0 16 16', fill: 'none', 'aria-hidden': true },
          h('path', {
    ...
    case 'file': return h(IconBrowseOutline16, { size, className })
    case 'folder': return h(IconFolderClose16, { size, className })
  }
}
