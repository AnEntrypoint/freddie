# @freddie/freddie-client-ui-brand-official

This package fills `sidebar.brand.mark`, `sidebar.brand.name`, and `conversation.hero.brand.mark` only when `FREDDIE_CLIENT_BUILD_PROFILE` is `official`. Other builds load the plugin but register no occupants, leaving the shell fallbacks visible.
...
- **The browser title is independent** — `FREDDIE_CLIENT_TITLE` selects title text at build time rather than through a UI slot.
