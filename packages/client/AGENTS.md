...
Packages here are named with the directory prefix: `@freddie/freddie-client-<name>`.
...
2. **A dynamic package declares internal dynamic relationships as peer plus dev.** Production source imports, re-exports, module augmentations, and type-only references to an `@freddie/freddie-*` package count, as does a package named by `dsh.client.inject`. A test-only internal dependency stays dev-only.
...
1. **Every GUI code change** — save and drive the running web GUI; watch `/plugins/events` and `window.__FREDDIE_HMR__`. There is no `test:gui` inner loop.
2. **Any change that can alter the assembled browser or visible conversation/UI output** (client components or copy, `apps/web`, Vite, `freddie-host-webserver`, connection/handler/SSE) — additionally `FREDDIE_SNAPSHOT=replay pnpm run test:web`: rebuilds the frontend dist, then runs the browser smoke pair (the real-host case self-skips without `DEEPSEEK_API_KEY`) plus the keyless replayed e2e scenarios. Linux PR CI uses the same read-only replay mode. Use `FREDDIE_SNAPSHOT=refresh` only after confirming an intentional output change, or `FREDDIE_SNAPSHOT=record` with a key to re-record fixtures.
3. **Before a PR** — use [freddie-pre-push-checks](../../.agents/skills/freddie-pre-push-checks/SKILL.md) to select the narrow checks for the outgoing diff; there is no repo-wide pre-push aggregate.
...
5. Save the source and watch `/plugins/events` plus `window.__FREDDIE_HMR__` on the running GUI. Buildless serving plus client-hmr reloads `src/client/` without `tsdown` or a package bundle.
...
5. Drive the running GUI; if the component changes visible assembled output, record that from the live page rather than a snapshot suite.
