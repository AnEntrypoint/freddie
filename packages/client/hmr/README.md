...
The browser half subscribes to the system SSE channel (`GET /plugins/events`), exposes connection and frame-sequence health at `window.__FREDDIE_HMR__`, and reloads one plugin per `rebuilt` frame through a serialized queue. The host sends an authoritative graph when each channel opens and periodic SSE comments while it stays open; failed, closed, or buffer-capped responses leave the broadcast set. Each rebuilt frame carries the host's updated graph row, which the browser applies before `invalidate` and `prefetch`, so native `import()` uses the new cache-keyed bundle URL. The remaining sequence is `registry.delete` (before the fiber: a bare fiber dispose trips the framework Loader's self-dispose branch, which would mark the entry disabled), drain the old fiber, delete `entry.fiber`, remove owned `<style data-plugin>` tags, `entry.refresh()` re-imports and re-applies, `fiber.await()` rethrows startup failures loud. A reconnect graph revision mismatch or a missing SSE frame reloads `AppWebEntry` under `/__hmr/<rev>/`. Dependents reload through Cordis itself: a fiber's activation epoch tracks its service providers' uids, so replacing a provider fiber cascades into dependents without client-side graph analysis. The node half debounces native filesystem save bursts before scanning a dirty dynamic, shell, or static root, retains polling only for roots whose platform cannot create a native watcher, and drops an SSE client whose pending socket buffer exceeds `maxBufferedSseBytes`; EventSource reconnects from the authoritative graph.
