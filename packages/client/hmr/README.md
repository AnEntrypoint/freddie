...
- **Graph changes that add or remove rows reload on reconnect** — rebuilt frames replace one existing row in place; a reconnect whose graph revision differs reloads the page rather than attempting a partial graph reconciliation.
