...
export class BootPage {
  ...
  constructor(container) {
    ...
    this.wordmark = div(css.wordmark, 'FREDDIE')
    ...
    this.card.append(this.wordmark, this.spinner, this.hint, this.progress, this.loadingDetail, this.current)
  ...
  render() {
    ...
    if (this.failure === undefined && failed.length === 0) {
      if (this.spinner.parentElement !== this.card) {
        this.card.replaceChildren(this.wordmark, this.spinner, this.hint, this.progress, this.loadingDetail, this.current)
      }
      return
    }
