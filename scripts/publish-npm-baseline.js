...
class DetachedWorktree {
  ...
  static create(repositoryRoot, commit, runner) {
    const temporaryRoot = mkdtempSync(join(tmpdir(), 'freddie-npm-baseline-'))
...
class ReleaseBundle {
  ...
  static load(manifestPath, runner) {
...
class InstalledBundleSmoke {
  ...
  run() {
    const consumerRoot = mkdtempSync(join(tmpdir(), 'freddie-npm-consumer-'))
    try {
      ...
      writeFileSync(resolve(consumerRoot, 'package.json'), `${JSON.stringify({
        name: 'freddie-npm-baseline-consumer',
        version: '0.0.0',
        private: true,
        dependencies,
      }, null, 2)}\n`)
  ...
  probeWeb(bin, consumerRoot, environment) {
    ...
    if (result.status !== 0) {
      throw commandFailure('python3', ['installed-freddie-web-probe'], result)
    }
  }
}
...
class RegistryPublication {
  ...
  remoteIntegrity(name) {
    ...
    const value = result.stdout.trim() === '' ? undefined : JSON.parse(result.stdout)
...
function inspectTarball(path, runner) {
...
function packedPackage(name, path, origin) {
...
function parsePackedPackage(value, index) {
...
function parseObject(source, context) {
...
function expectString(value, key, context) {
...
function installedArtifactEnvironment(consumerRoot) {
...
function validateBaseVersion(value, context) {
  if (!/^\d+\.\d+\.\d+$/.test(value)) {
    throw new Error(`${context} must have a stable X.Y.Z version, got ${value}`)
  }
}

function parseDistTagListing(raw, name) {
  const tags = new Map()
...
function formatCopyableCommand(command, args) {
