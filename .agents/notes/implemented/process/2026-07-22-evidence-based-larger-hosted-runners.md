...
The former gate-level and coarse primary shard jobs are absent from the workflow. Their workflow-facing static, lint, coverage, snapshot, and scenario selectors are also absent, so an unused diagnostic path cannot preserve a second CI architecture. The instrumented coverage gate and its process-local partitioning are gone along with the rest of the automated test suite; see [drop-automated-test-suite-testing-notes](../testing/2026-09-02-drop-automated-test-suite-testing-notes.md).
