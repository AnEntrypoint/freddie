...
- `build` used `tsc` to transform `.ts` to `.d.ts` files for packages under `packages/<group>/<pkg>` and the framework layer, and then used `tsdown` to transform `.ts` to bundled `.js` files. This made two tools do TypeScript transform.
- `typecheck` tended to validate packages, framework source, examples, tests, and scripts through one root typecheck config.
...
- The framework layer, examples, tests, and scripts cannot all be plain-included in one root strict program. At the time of this decision the framework packages were TypeScript, and typechecking their source under the root strict config triggered many type errors the harness did not own; package dependencies on them resolved to their built output, at a different tsconfig strictness. The framework layer has since become plain JavaScript with no `tsconfig.json`, no build step, and no `lib/` output, so it is no longer part of any TypeScript program and the boundary below concerns only `packages/*/*`.
...
- **One root strict program over packages, the framework layer, examples, tests, and scripts** — the framework source, then TypeScript, triggered type errors outside this project's ownership under the root strict flags; project references with per-project strictness are the boundary that works.
...
- Each ordinary module under `packages/<group>/<pkg>` has one local tsconfig for build, typecheck, and tools that run source directly, such as the `dsh` source loader, `tsx`, and `vitest`. `api/remotes` is the sole exception: generated-contract ordering requires one solution and two mutually exclusive emitting projects.
...
- The `typecheck` command uses `tsconfig.json`. Examples, tests, and scripts are checked by the root no-emit project, while packages keep the same emit behavior as `build`. Package source stays behind project-reference boundaries.
...
This added one more type-structure divergence between the Cordis framework packages and the code they descended from. That divergence is now moot: the framework layer is plain JavaScript maintained first-party, with no upstream sync step to reapply anything onto.
