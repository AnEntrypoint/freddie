...
`apps/cli/config/tui.cordis.yml` gained the `@freddie/freddie-tui/prompt` entry without a matching tsconfig `paths` mapping. The generic `@freddie/freddie-*` wildcard substitutes `tui/prompt` whole into its `<group>/*/src` candidates, none of which exist, so the tsx source launch of that era (superseded by the [buildless-workspace decision](../architecture/2026-09-02-buildless-workspace-no-transformation-at-launch.md)) fell back to package `exports` and resolved `lib/prompt.js` — an artifact-plane file. Every environment with a built `lib/` (developer trees after `pnpm build`) booted fine, and the e2e workflow runs the keyless TUI PTY smoke in `lib` mode (`FREDDIE_EXAMPLE_MODE=lib`, built bin under plain Node) so CI never exercises the source vector at all — while every clean checkout failed `pnpm dsh` at startup with `plugin(s) failed to load: @freddie/freddie-tui/prompt`. No gate checked the source plane, so the breakage shipped silently and surfaced only in fresh worktrees.
...
**A `@freddie/freddie-*/prompt`-style wildcard mapping.** Fixes this one subpath but not the class; the next single-file subpath export (`/surface`, `/message`, …) regresses identically. The static gate covers all current and future configured specifiers.
