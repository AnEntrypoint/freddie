...
`packages/*/*` and `apps/*` form the runtime surface of `@freddie/freddie`; `framework/*` holds nine rescoped Cordis framework packages, each on its own version line; `native/landlock-run/packages/*` holds Linux platform packages with their own workflow. The three differ in version baseline, change rate, and build requirements: dsh moves with the product, the framework layer moves only when its own sources change, and native needs a musl toolchain and one build per architecture. Forcing them through one pipeline means every product release republishes the framework and the native binaries.
...
`scripts/publish-npm-baseline.js` is a local publication script: it packs and publishes in one process, needs a human to authenticate and retry on their own machine, and excludes the framework layer from its release set. It cannot be the basis for CI publication, though its tarball payload validation and installed-artifact probes are verified parts.

## Decision

### Three independent sequences

`packages/`, `framework/`, and `native/` each have one bump sequence and one publication, sharing no version, no trigger, and no waiting. Releasing dsh does not republish the framework layer; releasing the framework layer does not republish native.
...
The framework packages are first-party and published under their own scope, but each keeps its own version line, continuing from the version it forked at. The published version is the higher of the manifest version and the last published version, with the patch incremented — which also drops an inherited prerelease segment. The first published versions:

| Package | Inherited version | First published version |
|---|---|---|
| `@freddie/cordis` | 4.0.0-rc.7 | 4.0.1 |
| `@freddie/cordis-plugin-loader` | 1.0.0-rc.5 | 1.0.1 |
| `@freddie/cosmokit` | 1.8.1 | 1.8.2 |
| `@freddie/schemastery` | 3.18.0 | 3.18.1 |
| `@freddie/cordis-plugin-hmr` | 1.0.15 | 1.0.16 |
| `@freddie/cordis-plugin-include` | 1.0.4 | 1.0.5 |
| `@freddie/cordis-plugin-timer` | 1.1.2 | 1.1.3 |
| `@freddie/cordis-plugin-group` | 1.0.0 | 1.0.1 |
| `@freddie/cordis-plugin-logger-console` | 1.0.0 | 1.0.1 |

Taking the last published version as the baseline is what survives a manifest version moving backwards: a manifest restored to `4.0.0-rc.8` after this repository published `4.0.1` would otherwise compute `4.0.1` again and collide. `--prerelease rc.1` publishes a rehearsal instead, which takes `--tag next` and leaves the release numbers free: a prerelease has lower precedence than the release it precedes, so `4.0.1` still follows `4.0.1-rc.1`. That ordering is computed here rather than read from `git tag --sort=v:refname`, which places a prerelease above its release.

Only changed packages publish, and the change judgement adds no state file: **each package has its own tag, and that tag records the commit it last published from**. For each package, bump reads the newest `vendor-<package>-v*` tag and diffs the package directory against it. A path counts when the manifest's `files` selects it, when npm publishes it regardless (`package.json`, `README*`, `LICENSE*`), or — for a package whose `files` selects build output rather than source — when it is a build input. That last rule exists because a built payload is not tracked by git: without it, a real source change reads as "nothing changed" and the next publication fails on a version whose bytes moved. It does not apply to the framework packages, which have no build step: their `files` select `src/**` directly, so every source change is already a tracked path.
...
`framework/cordis` publishes `src` as well. Its export map declares `"./src/*"`, so a tarball without those files points consumers at absent paths, and `files` selecting only build output left the change judgement with no tracked path to match.
...
An optional-dependency import gate closes that hole. It reads each package's own manifest for what that package allows to be absent, then scans the files that ship — `packages/*/*/src/` and `apps/*/src/` — across both compiler faces. `framework/` is out of scope: it is the first-party framework layer, which the harness declares as peers rather than shipping from `packages/*/*/src/` or `apps/*/src/` ([framework layer](../../../../framework/README.md)). Value-versus-type is decided against a bound Program rather than the import syntax, because `verbatimModuleSyntax` is off: the compiler already erases an import whose bindings resolve to types, so `import type {}`, `import {}`, an inline `type` specifier, and a named binding that resolves to a type all emit nothing and are allowed, while a bare import, a value binding, and a star re-export are kept and rejected. Only the type phase erases an import: `import defer` still resolves and links its module, deferring evaluation alone, so the gate counts it as a load.
...
The dsh family applies the repository's publication payload policy, which rejects sources and declaration maps. The framework family publishes its source instead, because those manifests export `./src/*` and dropping `src` would publish an export map pointing at absent files.
...
A dsh verification installs the framework family's pack output too. The harness packages declare the framework as a peer, those packages live in another sequence, and the credential-free job cannot fetch them from a private registry — so the dsh `pack` job packs the framework family for verification while publishing only the dsh set. The publish workflow (`release-publish.yml`) repacks the current tree and publishes only the dsh set.
...
| release-set boundary | every member of `packages/*/*`, `apps/*`, and `framework/*` |
| dependency protocol | workspace-internal references are `workspace:^`, with the workspace-constraints check and the invariant-companion rule requiring it |
| root `AGENTS.md` | the convention that framework packages are `private: true` no longer holds |
| `framework/README.md` | records `src` joining `cordis`'s `files` in the divergence log |
...
**Putting the nine framework packages on one `4.0.x` line.** It removes change detection, but cosmokit would jump from `1.8.1` to `4.0.1` and lose the version lineage it continues from; the inherited ranges inside the nine (`^1.8.1` and friends) would stop matching immediately, forcing a rewrite of the framework manifests.

**Incrementing every framework package on every release of that family, with no change detection.** The least machinery, at the cost of new version numbers for packages whose content is byte-identical to the previous release. Tags reduce change detection to reading one tag and running one diff, which is not worth trading for inflated version numbers.
...
- **The change judgement depends on visible tags.** A shallow clone, or a checkout without tags, degrades the framework family's judgement to "publish everything for the first time". `fetch-depth: 0` is a precondition, not an optimization.
...
- **The first publication is one large step.** Nine framework packages and the whole dsh set publish at once, so any payload defect surfaces in a single release, which is why a prerelease version drives the complete path first.
