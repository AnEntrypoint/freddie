...
| Package | Role |
|---|---|
| `@freddie/freddie-spill` | Interface: `ctx.spillStore`, vocabulary types, no storage implementation. |
| `@freddie/freddie-spill-local` | Local backend: private, session-scoped file storage on the host filesystem. |
| `@freddie/freddie-spill-policy` | Tool-result policy plugin: wraps final text results after dispatch and replaces oversized results with a retained preview plus a spill locator. |
...
- id: spill-local
  name: '@freddie/freddie-spill-local'

- id: spill-policy
  name: '@freddie/freddie-spill-policy'
  config:
...
- `@freddie/freddie-output-retention` owns preview mechanics (`TextRetainer`, `ItemRetainer`, and omitted metadata).
- `@freddie/freddie-spill` owns saving final text and returning a locator plus retrieval hint.
- `@freddie/freddie-spill-policy` applies the default final-result policy in the tool pipeline, composing the two.
