...
- `index` (the default chunk) = the react family (react, react-dom, scheduler, use-sync-external-store), the cordis framework, all workspace code, and the unlisted small pieces (anser, clsx).
