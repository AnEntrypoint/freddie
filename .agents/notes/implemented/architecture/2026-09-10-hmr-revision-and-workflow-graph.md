...
`ctx.workflowEngine.graphs` records leaf-only run graphs from `workflow/*` events so parallel dynamic workflows have native tracking without dumping live Cordis objects.
