...
The GM verbs remain host capabilities until a deployment-owned evaluator provider produces authenticated target-bound outcome receipts. `@freddie/freddie-tool-gm` does not expose construction or replay to models while that provider is absent. `@freddie/freddie-dream-rsi-context` defaults to disabled and installs no request listener until a deployment explicitly enables it with that provider. When enabled, it derives a replay receipt from the preceding durable tool-call/result relation and injects a sourced directive only when the receipt exactly matches the replay call and validated successful result. The directive identifies the selection, scores, bounded observed-node evidence, and its grounding limit. The plugin accepts only opaque identifiers before rendering them, and it neither reads GM files nor invokes GM nor authorizes candidate-policy effects.
