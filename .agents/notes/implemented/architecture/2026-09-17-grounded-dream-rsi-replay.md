...
`@freddie/freddie-tool-gm` exposes sealing as `gm_dream_world_seal` and evaluation as `gm_dream_replay`. Their normal tool-call and tool-result session records remain the authoritative replay receipt; `gm/progress` continues to record dispatch lifecycle state. The replay tool writes private presentation metadata binding its exact policy and world IDs to the reported selection. `@freddie/freddie-dream-rsi-context` is an opt-in pre-step plugin. It derives a replay receipt from the preceding durable tool-call/result relation and injects a sourced directive only when the receipt exactly matches the replay call and validated successful result. The directive identifies the selection, scores, bounded observed-node evidence, and its grounding limit. The plugin accepts only opaque identifiers before rendering them, and it neither reads GM files nor invokes GM nor authorizes candidate-policy effects.
