...
`JobView` is browser-safe and owned by the carrier at [`packages/host/apiproxy/src/api/jobs.ts`](../../../../packages/host/apiproxy/src/api/jobs.js), alongside the other domain contracts, with its wire schema beside it in `jobs.schema.ts`:

```ts
import type { JobId } from '@freddie/freddie-jobs/brand'
...
`JobId` comes from the cordis-free [`@freddie/freddie-jobs/brand`](../../../../packages/jobs/jobs/src/brand.js) leaf — the same arrangement as the `@freddie/freddie-llm/brand` import `api/subagents.ts` already uses, because the `dsh-jobs` root reaches `dsh-agent` and is unreachable from a client program even as a type. Like every other non-root subpath in this workspace, it carries an explicit `tsconfig.base.json` `paths` entry; without one the Typert analyzer resolves the specifier to `lib/types/` and rejects the reference as unexported.
...
The frame carries a whole snapshot rather than a delta for the reason [`session/queue`](../../../../packages/host/apiproxy/src/api/events.js) states for itself: start, kill, settlement, reconnect, and a second browser tab all converge through one authoritative value. A session's task set is single-digit; the frame is small.
