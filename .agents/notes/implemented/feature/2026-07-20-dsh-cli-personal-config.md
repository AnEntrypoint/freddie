...
**The `dsh` CLI (`apps/cli`, npm name `@freddie/freddie`).** `apps/*` is the product-assembly tier over `packages/*` libraries. One bin dispatches the default interactive TUI, `-p`/`--prompt` headless turns, and the `web` surface. The TUI boots `examples/tui-agent/cordis.yml` (or `--config`) with the invoking directory as the workspace. The root `pnpm dsh` script runs the same entry directly under plain `node` — the whole workspace is buildless JavaScript, so there is no transformation step between source and the running process; the [buildless-workspace decision](../architecture/2026-09-02-buildless-workspace-no-transformation-at-launch.md) owns the runtime vector and artifact generation.

**Personal config (`dsh-app-boot`).** The personal overlay lives in the Harness home — `$FREDDIE_HOME`, else `~/.dsh` — resolved by the shared [`resolveDshHome`](../architecture/2026-07-24-single-harness-home-resolver.md) (`@freddie/freddie-home-paths`), the same single root skills and AGENTS.md resolve against. The dsh TUI, Web, and headless surfaces consume its two optional files; the demo bins boot their committed trees verbatim:
...
The PTY smoke's launcher isolates `$FREDDIE_HOME` to a per-test directory, exactly as it already isolates `FREDDIE_AGENTS_HOME`, so a developer's real personal overlay cannot leak into fixtures; only the dsh CLI reads personal config, so no other test launcher needed changes.
...
- `dsh-app-boot` depends on `js-yaml` and imports the include's `!!js` YAML dialect (`entryListSchema`) directly, and, like `apps/cli`, depends on `@freddie/freddie-home-paths` for `resolveDshHome`.
...
`packages/boot/app-boot/tests/user-patches.spec.ts` pins parsing, startup application, exact-path add/failure/recovery/removal, last-good rollback, failure broadcast, and preservation of app-owned patches. `apps/cli/tests/built-bin.e2e.ts` boots the real dsh bin over a profile and exercises the live patch layer end to end. Test launchers isolate `$FREDDIE_HOME`, so a developer's real overlay cannot leak into fixtures.
