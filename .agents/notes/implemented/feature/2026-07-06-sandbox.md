...
```yaml
- id: sandbox
  name: '@freddie/freddie-sandbox-local'   # the per-platform runner provider (ctx.sandbox)
- id: bash
  name: '@freddie/freddie-bash-sandbox'    # the confined executor, replacing dsh-bash-local behind ctx.shell
  config:
...
- id: approval
  name: '@freddie/freddie-user-approval'        # the escalation gate's channel (the approval Agent Note)
  config:
    policy: ask
- id: permission
  name: '@freddie/freddie-permission-presets'      # one product-facing select over both mechanism knobs
```

The swap is invisible to every consumer of `ctx.shell`: the bash tools, hook commands, and background jobs run exactly as before by directly spawning the wrapped argv the provider returns. Deleting the `sandbox` and `permission` entries and replacing `bash` with `@freddie/freddie-bash-local` is the opt-out — execution is unconfined again and the escalation fields vanish from the tool schema, because they are capability-gated on the mounted executor, not on configuration. Omitting only `approval` keeps confinement but fails every escalation closed with its own error text; `permission` also requires the approval seam and a confining executor, so a partially composed preset layer fails loud at load.
...
Denied file effects return a `[sandbox: file access denied under <mode> mode]` marker and instructions not to work around the denial. A confining executor adds paired `sandbox_permissions` and `justification` fields. A request wider than the session's effective mode needs one approval and widens only that retry; a request at or below the standing mode is a no-op. Rejection executes nothing, returns `the user rejected escalating this command to "<mode>"`, and permits no re-ask. The owner-derived pending policy context states the current file policy without replacing those enforcement boundaries. When `dsh-permission-presets` is composed with a UI adapter, one preset selects both knob values; unmatched values fold to `custom`. The [ACP automation composition](../../../../examples/acp-agent/README.md) does not mount that UI service and selects its deployment mode explicitly.
...
- **Windows chain** — `PLATFORM_CHAINS.win32` is reserved and empty (fail-closed); filling it means a confinement runner from the AppContainer/restricted-token family, shipped from the main repository under `native/` following the `@freddie/node-addon-landlock-run` template, plus its profile dialect, denial signatures, and runner-failure rules. Wrapping the third-party landstrip runner instead was [considered and rejected](../../rejected/feature/2026-07-26-evaluate-landstrip-for-windows-sandbox-rung.md) — not battle-tested enough for a security invariant.
...
- A denied command retried with `sandbox_permissions` + `justification` prompts the user through the composed answerer chain when the requested mode is wider; a grant runs THAT call under the wider mode (result facts say so) while every other call keeps its own effective mode. A request at or below the standing mode executes under that mode without an approval prompt; every non-grant wider request produces its distinct error text and executes nothing.
- The escalation fields exist exactly when the mounted executor confines; a request at or below the call's effective mode retains that mode and prompts no one; a deployment with no ApprovalService fails wider requests closed and leaves plain calls untouched.
...
- **The advertised target set is static while the effective mode is per-session** (schemas are registry-global) — a session already at the widest mode is still offered the fields. Harmless by construction: execution retains the standing mode for a non-widening request, while only a wider request reaches the approval boundary.
