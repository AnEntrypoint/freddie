...
- **`@freddie/freddie-pwsh-local`** — a local implementation of the `ctx.shell` executor seam over `ctx.subprocess`, mirroring `dsh-bash-local` call-for-call: `resolve()` defaults and caps from config, `run()` fuses the config-clamped timeout with the caller's signal through one deadline, `start()` returns a consuming background handle whose processes belong to the subprocess service. The command string rides as ONE argv element to `pwsh -NoLogo -NoProfile -NonInteractive -Command`, so PowerShell parses it and no shell-quoting layer exists. Executable resolution (`resolvePwshPath`) is a pure function of `(configured, env, platform)`: explicit config first, then Windows probes PowerShell 7's install, PATH entries (quotes stripped), and Windows PowerShell 5.1, else a bare `pwsh` via PATH.
- **`@freddie/freddie-tool-pwsh`** — the model-facing tool over `ctx.shell`, PowerShell-dialect by contract, mirroring `dsh-tool-bash` call-for-call: foreground and `run_in_background` execution through the generic job runtime, managed `FREDDIE_*` environment through the shared [`dsh-shell-env`](../feature/2026-08-02-pwsh-tool-bash-parity.md) registry, the bash marker/truncation rendering story (a clean exit produces no marker), and — since the Windows ACL sandbox decision — the sandbox denial rendering and `sandbox_permissions` escalation surface, plus the Windows-specific ConstrainedLanguage and named-pipe contracts in the tool description. The description names the executor's resolved executable (`ctx.shell.pwshPath`): `pwsh -Command` when PowerShell 7 is present, and `powershell.exe -Command` (Windows PowerShell 5.1 last-resort) when it is not, so a host without `pwsh` is not taught a false engine. The parity decision supersedes this note's minimal-profile tool description.
