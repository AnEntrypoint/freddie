...
The `workflow` tool supports foreground collection and an owner-controlled background job. Foreground execution starts, awaits, and disposes in `finally`; a non-`completed` outcome becomes `isError`. Background execution registers the live run in `ctx.jobs`, returns a job id, and keeps recording lifecycle events until the holder reaches quiescence. `job_output` observes settlement and `job_kill` invokes the run cancellation path. Render intent remains a generic card titled from `meta.name`. The tool description is the model-facing authoring spec, and its explicit-ask policy ships as the `tool:<toolName>` prompt section.
...
- **Background execution as the default** (CC's shape): rejected; explicit `run_in_background` preserves foreground result semantics while the shared job runtime owns durable collection and cancellation.
...
Fan-out plans live in rerunnable scripts, and `outputSchema` provides authoritative structured child results. Each run pays worker startup and message-port RPC costs, while host startup remains non-blocking, cancellation can terminate the worker, and serialization enforces the value boundary. Worker threads are not a security boundary. Invalid options fail rather than degrading to Claude Code's `null`; foreground consumers retain the run handle, and background consumers transfer lifecycle ownership to the shared job runtime. Top-level Web users receive a durable, replayable workflow record without widening the execution seam or coupling the original tool card to workflow-specific UI.
