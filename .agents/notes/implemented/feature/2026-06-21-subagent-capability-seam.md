...
The harness has a long-deferred seam for **subagents** — an agent delegating work to another agent. The intent was sketched in the `Agent`/`AgentLoop` interfaces ([packages/core/agent/src/types.ts](../../../../packages/core/agent/src/types.js), [packages/core/agent-loop/src/index.ts](../../../../packages/core/agent-loop/src/index.js)): a creation option referencing a parent agent (fork = seed the child session with the parent's event log; spawn = fresh session), with the child returned as an `Agent` handle so steering and event subscription work uniformly.
...
| Package | Role |
|---|---|
| `@freddie/freddie-subagent` | interface: `SubagentRuntime` (`ctx.subagents`), `SubagentProvider`, `SubagentRun`, the request/result/capability vocabulary, the `subagent/*` events |
| `@freddie/freddie-subagent-spawn-in-process` | implementation: a fresh in-process child via `ctx.agents.create` |
| `@freddie/freddie-subagent-fork-in-process` | implementation: an in-process child seeded with a snapshot of the parent's log |
| `@freddie/freddie-subagent-acp` | implementation: an ACP client driving a configured child process |
| `@freddie/freddie-subagent-codex` | implementation: a one-shot official Codex app-server process |
| `@freddie/freddie-subagent-claude-code` | implementation: a one-shot official Claude Code process through the Agent SDK |
| `@freddie/freddie-tool-subagent` | consumer: the model-facing `subagent` tool over `ctx.subagents` |
