...
That lifecycle is not required to run or develop Freddie from a source checkout. Maintaining it expands the supported filesystem and Git state space without improving the repository-native execution path.

## Decision

The repository supports source execution through its root `pnpm` scripts. The `dsh` entry in `package.json` launches `apps/cli/src/bin.js` directly under plain `node` — no transformation step, per the [buildless-workspace decision](../architecture/2026-09-02-buildless-workspace-no-transformation-at-launch.md); artifact generation is the separate `pnpm run build` operation. The package script forwards arguments and inherits the caller's environment, including `NODE_USE_ENV_PROXY=1` when a supporting Node version must honor `HTTP_PROXY` and `HTTPS_PROXY`. Users select Web with `pnpm dsh web` and headless execution with `pnpm dsh --profile headless "task"`. The independent ACP example remains available through `pnpm run demo:acp`.
