...
The repository carried an unreleased developer-project product with no consumers. `@freddie/create-sdk` generated an editable Cordis project, `@freddie/freddie-scripts` supplied its `dsh-sdk` development, build, start, configuration, and plugin-install commands, `@freddie/freddie-helper` coordinated feature definitions and multi-file project edits, and `@freddie/freddie-telemetry` reported launcher activity. The design aimed to keep generated projects editable while giving creation and later configuration one definition of dependencies, Cordis entries, environment placeholders, and owned files.
