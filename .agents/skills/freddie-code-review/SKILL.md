---
name: freddie-code-review
description: Use when reviewing a pull request in the freddie repo — orients the reviewer to this codebase's standards (AGENTS.md conventions, defensive patterns, ADRs, quality gates) and the review-specific checks that code alone can't show
---
...
- [freddie-prose-standard](../freddie-prose-standard/SKILL.md): required coverage and editorial judgment for comments, docs, prompts, and visible strings.
- The [quality-gates Agent Note](../../notes/implemented/process/2026-06-11-quality-gates.md) and root `AGENTS.md`'s verification-policy convention: live verification, no automated test suite.
...
1. **New prose receives semantic review.** Use [freddie-prose-standard](../freddie-prose-standard/SKILL.md) to critically review every added or changed Markdown passage, JSDoc, comment, prompt, description, diagnostic, and visible string. Verify required coverage, accuracy, placement, and editorial quality against the owning code or behavior; automated checks do not establish those properties.
