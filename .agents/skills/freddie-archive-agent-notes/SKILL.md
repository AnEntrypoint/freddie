---
name: freddie-archive-agent-notes
description: Use when adding, auditing, pruning, archiving, restoring, or reviewing Agent Notes in freddie; checks every new note for superseded active records, classifies implemented notes by future decision value, deletes rejected notes that no longer prevent a tempting fallacy, and applies the frozen archived/{kind} and manifest rules.
---

# Archive Freddie Agent Notes
...
- **Rejected — delete:** delete the file when the rejected idea is obsolete, superseded, no longer plausible, or unlikely to prevent re-litigation. Repair or delete inbound links.
...
This repo has no automated Agent Note format/archive verifier; validate by re-reading the moved file against the [format rules](../../notes/README.md#the-file-format) and searching the repo for the note's old path.
