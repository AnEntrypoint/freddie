...
- Read `AGENTS.md`, especially the pre-release stance and the conventions (including the Agent Notes-are-not-golden-truth doctrine), plus [docs/defensive-patterns.md](../../../docs/defensive-patterns.md).
...
Thin candidates are usually not enough for an Agent Note: deleting one typo, a single unverified grep pass, removing an intentionally documented backend/adapter, or flagging "this looks complex" without call-site proof.
...
Use the project's own code-search surface first. Good searches include the exact symbol, event name, package name, config key, method name with both `.name(` and `name(`, and any wire strings. Then read the call sites — no automated dead-export tool is a substitute for understanding public interfaces, dynamic event names, docs, and Cordis loader paths.
...
Use [`freddie-archive-agent-notes`](../freddie-archive-agent-notes/SKILL.md) for retention judgment and archive mechanics. Low-future-value implemented notes move to frozen `archived/{kind}`; proposed notes are never archived; rejected notes that no longer prevent a tempting mistake are deleted. Do not edit an archived note while simplifying current prose or code.
