...
# export FREDDIE_MODEL=deepseek-v4-flash
# export FREDDIE_SYSTEM_PROMPT='You are a helpful software engineer assistant.'
```
...
| Property | Value |
|---|---|
| System prompt | `FREDDIE_SYSTEM_PROMPT`, falling back to `You are a helpful software engineer assistant.` |
| Model in `minimal.py` | `--model`, then `FREDDIE_MODEL`, then `deepseek-v4-flash` |
...
| Session persistence | Uncompressed JSONL under `FREDDIE_SESSION_ROOT` |
