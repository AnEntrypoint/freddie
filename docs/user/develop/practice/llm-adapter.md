...
import { LlmAdapter, type GenerateOptions, type StreamChunk } from '@freddie/freddie-llm'
...
```ts
import { CallId, type StreamChunk } from '@freddie/freddie-llm'
...
`stream()` receives the exported `GenerateOptions` type. It includes the model, adapter-owned reasoning-effort id, conversation history, system prompt, tool schemas, generation parameters, stop sequences, and abort signal; treat the TypeScript type exported by `@freddie/freddie-llm` as authoritative. Map supported fields to the provider API. If the provider cannot honor a field, throw `LlmError` with a stable code instead of silently dropping it.
...
- id: agent-loop
  name: '@freddie/freddie-agent-loop'
  config:
    agents:
      - id: main
...
The repository contains a complete implementation: `packages/llm/llm-deepseek/` — a DeepSeek API adapter using the OpenAI-compatible format.
