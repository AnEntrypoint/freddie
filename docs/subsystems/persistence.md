...
Repair applies only to cold sessions. For a live id, `SessionPersistence.load(id)` waits until the authoritative in-memory snapshot is durable and returns it only when balanced; an open live turn rejects rather than receiving synthetic interruption boundaries. HMR adopts a live prefix without closing its active turn. After `ctx.agents.resume` publishes a repaired session whose last `turn/end` is `{ kind: 'interrupted' }`, the agent-loop queues a plugin `notice` follow-up so the unfinished work continues without a further human prompt ([agent-loop](../../packages/core/agent-loop/README.md)); a live `{ kind: 'aborted' }` cancel is not overridden.
