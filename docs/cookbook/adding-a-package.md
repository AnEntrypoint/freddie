...
The file-by-file checklist for a new `@freddie/freddie-<name>` package. This checklist is validated against the bash and adapter packages as templates; if it drifts from them, fix it here.
...
packages/<group>/<pkg>/
  ...
  src/index.js     # service default export or plugin (name/inject/apply/Config)
...
The repository is buildless plain JavaScript (ESM, `"type": "module"`) — there is no `tsconfig.json`, no `lib/` output, and no separate `types` field. package.json invariants, following `packages/core/tools/package.json` as the template: `private: true` (unless the package is meant to publish), a `version` matching the root `package.json`, `main: "src/index.js"`, `exports["."].default: "./src/index.js"` (plus a `./src/*` passthrough and any other real subpath exports the package provides), `@freddie/cordis` in BOTH peerDependencies and devDependencies (same range). Mirror every dsh peer dependency in devDependencies. `@freddie/schemastery` goes in `dependencies` when it's a runtime validator for the package's `Config`. The `files` array lists the real glob(s) needed for the package's exports to resolve when published (typically `src/**/*.js`) — `pnpm run publint` fails the package if an import reaches a file the `files` array doesn't list.

In-package relative imports use `.js` specifiers directly in source (for example, `export * from './types.js'`) — there is no compiler rewriting specifiers, so the specifier in source is exactly the specifier Node resolves at runtime.

## 2. Register it in the workspace

A new package under an existing group needs no registration beyond its own `package.json` — the root `package.json` `workspaces` globs (`packages/*/*`) pick it up automatically on the next `pnpm install`. A `packages/client/*` package additionally declares `dsh.client` in package.json and exports `./client` — see [packages/client/AGENTS.md](../../packages/client/AGENTS.md) for the client-side contract.
...
Use `SDK` only for the JSON-RPC client/server protocol used by the supported Python and TypeScript SDKs. Freddie itself is an agent harness, not an SDK project. Use the canonical product spelling `Typert`, never `TypeRT` or `typeRT`.
...
Fill Model Experience from the implementation. Use one H3 per direct, conditional, capped, lifetime, or auxiliary model-context entry, with the three ordered H4 fields shown above and one prose paragraph under each. Quote stable text owned by the package: system-prompt prose goes in a titled H5 plus `markdown` fence under the field that introduces it—normally `What the model sees`—other short literals stay inline with named placeholders, and other long literals use the same nested form. Summarize only data-dependent or provider-owned text. A tool-schema entry links its anchored section in the generated [tool catalog](../tool-catalog.md) and states only deltas absent there. Keep prompt and schema entries separate when scoping can hide one without the other. In `KV Cache effect`, distinguish append-only growth, a stable repeated prefix, replacement of earlier request tokens, and an independent model request, then name the package-owned changes that can invalidate reuse. “Does not invalidate” means the package preserves an already-reusable prefix; provider cache availability and eviction remain outside the package contract. The [prose standard](../../.agents/skills/freddie-prose-standard/SKILL.md) governs completeness and ownership; the verifier enforces the required section structure.

A package with no context effect or one consumer-owned path uses the audited `None, as ` or `Indirectly, through ` sentence, followed by a `KV Cache effect` H4 and one non-empty paragraph; a model-agnostic generic package may instead join `NO_MODEL_EXPERIENCE_SECTION`. Do not expand either case into a description of another package's work. A separate limitations allowlist governs which packages may omit this section; both the sentence source and the limitations allowlist were previously enforced by `scripts/verify-package-readme-model-experience.ts` and `scripts/verify-package-readme-limitations.ts`, neither of which exists in this checkout — treat the rule as documentation-only until a verifier is restored. The [Model Experience Agent Note](../../.agents/notes/implemented/process/2026-07-12-package-model-experience-contract.md) records the rationale.
