...
- The native-boundary regression cases previously lived in `packages/shell/bash-sandbox/tests/partial-landlock.spec.ts` (no `tests/` directory exists in this checkout — the coverage described here is not currently present as a file), including informational notices, fatal evidence, and foreground/background classification.
