...
// Adds message_count and last_content (the most recent message's raw
// content, untruncated -- callers decide their own display truncation) to
// each listSessions row via a correlated scalar subquery per session, so a
// resume picker can render "id, N messages, last: <preview>" per row
// without a second round-trip per session. Bounded by the same limit as
// listSessions -- never a full-table scan of every session ever created.
export async function listSessionsWithSummary(limit = 50) {
    const d = await db()
    return await d.prepare(`
        SELECT s.id, s.platform, s.title, s.created_at, s.updated_at, s.model, s.cwd, s.skill, s.parent_id,
            (SELECT COUNT(*) FROM messages m WHERE m.session_id = s.id) AS message_count,
            (SELECT m.content FROM messages m WHERE m.session_id = s.id ORDER BY m.ts DESC, m.id DESC LIMIT 1) AS last_content
...
export async function deleteSession(id) {
    ...
    // The transaction callback receives the SAME connection BEGIN/COMMIT ran
    // on as its own final argument (db.js's transaction() wrapper) -- every
    // nested statement MUST run against that connection, never a fresh
    // d.prepare() call, or it silently escapes the transaction entirely
    // (db.js is open-per-call by design; a call outside the tx callback's
    // own connection opens an unrelated connection that never sees the
    // BEGIN, defeating the whole point of wrapping these in one).
    const run = d.transaction(async (embedded) => {
        await embedded.raw(`DELETE FROM messages WHERE session_id = ?`, [id])
        try { await embedded.raw(`INSERT INTO messages_fts(messages_fts) VALUES('rebuild')`) } catch { /* swallow: FTS rebuild is best-effort */ }
        return await embedded.raw(`DELETE FROM sessions WHERE id = ?`, [id])
    })
    const info = await run()
    return { id, deleted: (info.rowsAffected ?? 0) > 0 }
}
...
export async function purgeSessionMessages(id) {
    const d = await db()
    const run = d.transaction(async (embedded) => {
        const info = await embedded.raw(`DELETE FROM messages WHERE session_id = ?`, [id])
        try { await embedded.raw(`INSERT INTO messages_fts(messages_fts) VALUES('rebuild')`) } catch { /* swallow: FTS rebuild is best-effort */ }
        return info
    })
    const info = await run()
    return { id, deleted: (info.rowsAffected ?? 0) }
}
