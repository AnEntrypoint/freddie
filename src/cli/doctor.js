...
const CHECKS = [
    ...
    // ESM-only package (no "require" export condition) -- require.resolve()
    // always throws ERR_PACKAGE_PATH_NOT_EXPORTED for it regardless of
    // whether it's actually installed, so this check needs a real dynamic
    // import probe instead of the require.resolve() pattern the other
    // module-presence checks use.
    { name: 'libsql-plugkit-client', run: async () => { try { await import('libsql-plugkit-client'); return { ok: true } } catch { return { ok: false, fix: 'npm install' } } } },
    ...
    { name: 'gm-learn', run: async () => {
        ...
        try {
            ...
            const doctorAbort = new AbortController()
            const hits = await Promise.race([
                recall('doctor healthcheck probe', { limit: 1, signal: doctorAbort.signal }),
                new Promise((_, reject) => setTimeout(() => { doctorAbort.abort(); reject(new Error('timed out probing gm-learn backend')) }, GM_LEARN_DOCTOR_TIMEOUT_MS)),
            ])
