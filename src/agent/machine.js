...
export async function runTurn({ prompt, messages = [], model, provider, callLLM, enabledToolsets, disabledToolsets, maxIterations = 90, timeoutMs = 30000, cwd, skill, witnessPath, sessionKey, toolCtx = null, tool_choice, store, approvalMode = null, approvalTimeoutMs = null } = {}) {
    ...
    try {
        ...
        const autorecallAbort = new AbortController()
        let autorecallTimer
        const hits = await Promise.race([
            (async () => autoRecall(prompt, { limit: 5, namespace: await projectNamespace(), signal: autorecallAbort.signal }))().finally(() => clearTimeout(autorecallTimer)),
            new Promise((_, reject) => { autorecallTimer = setTimeout(() => { autorecallAbort.abort(); reject(new Error('autoRecall timeout')) }, AUTORECALL_TIMEOUT_MS) }),
        ])
    ...
    const mergedEntry = mergeTurnEntry(key, { actor: pa.actor, control, abortController })
    if (!mergedEntry) {
        return { messages, result: null, error: `turn no longer live for session ${key}`, iterations: 0 }
    }
    // A cancelTurn() call landing during setup (before actor/abortController
    // existed) already fired the deferred INTERRUPT above (mergeTurnEntry,
    // turn-registry.js) -- but INTERRUPT before the machine's first SUBMIT is
    // a no-op (the machine hasn't entered a state that handles it yet), and
    // this function was about to send SUBMIT unconditionally regardless,
    // starting the turn anyway. Live-witnessed defect this closes: a cancel
    // that arrived early recorded cancelRequested=true and returned true from
    // cancelTurn(), yet the turn still ran to completion uninterrupted because
    // nothing here checked the flag before dispatching SUBMIT. Short-circuit
    // to the same cancelled shape driveAgentActor produces on a live cancel.
    if (mergedEntry.cancelRequested) {
        unregisterTurn(key)
        return { messages, result: null, error: 'turn cancelled', iterations: 0 }
    }
