...
export async function chat({ messages, model = 'anthropic.claude-sonnet-4-v1:0', tools = [] } = {}) {
    const id = (await resolveKey('aws')).value || env('AWS_ACCESS_KEY_ID')
    ...
    const r = await fetch(url, { method: 'POST', headers: { authorization: 'AWS4-HMAC-SHA256 ' + (env('AWS_SESSION_TOKEN') || ''), 'content-type': 'application/json' }, body })
    return await r.json()
}
