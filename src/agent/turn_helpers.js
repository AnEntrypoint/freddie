...
function timeoutResult(actor, timeoutMs) {
    ...
    // resumable:true tells a caller its persisted snapshot survives this
    // timeout (turn_driver.js no longer clears it) and resumeTurn({sessionKey})
    // can continue from exactly where the xstate machine left off -- a
    // dedicated flag rather than string-matching error, so callers don't need
    // to know/rely on the exact wording of the timeout message.
    return { messages, result: null, error: 'agent turn timeout', resumable: true, iterations: ctx.iterations || 0 }
}
