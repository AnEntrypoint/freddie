...
function parseMermaid(content) {
    ...
    remapTerminalIds(nodes)
    return { nodes, edges, format: 'mermaid' }
}

function remapTerminalIds(nodes) {
    for (const id of Object.keys(nodes || {})) {
        const n = nodes[id]
        if (!n) continue
        const k = String(id).toLowerCase()
        if (k === 'begin' || k === 'start') n.type = 'start'
        else if (k === 'end' && n.type !== 'decision' && n.type !== 'start') n.type = 'end'
    }
}
