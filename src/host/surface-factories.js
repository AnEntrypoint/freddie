    ...
    return {
        ...
        async dispatchTool(name, args = {}, ctx = {}, opts = {}) {
            ...
            try {
                ...
                // Tools that already return a string (including the plain-JSON
                // early-return error paths above, e.g. 'unknown tool: X') pass
                // through unchanged -- machine_builder.js's unknown-tool
                // detection JSON.parses that exact shape and must keep working.
                // Only a plain-object handler result gets serialized here, and
                // it's done as YAML (not JSON) so every tool's wire format is
                // consistent with the ones (read, wait, gm) that already
                // return YAML directly.
                // yaml.dump throws on a handful of shapes JSON.stringify
                // tolerates silently (a bare Map, notably) -- fall back to
                // JSON rather than letting a valid tool result get discarded
                // and replaced with a generic {error} by the outer catch.
                let raw
                if (typeof r === 'string') raw = r
                else { try { raw = yaml.dump(r, { lineWidth: 100 }) } catch { raw = JSON.stringify(r) } }
