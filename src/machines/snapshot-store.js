...
// to a fresh actor and never throws. That "never throws" guarantee used to be
// only partially true: the DB read itself (init()/prepare()/get(), below) had
// no try/catch of its own, so a backend-level failure (live-witnessed: the
// libsql-plugkit-client wasm backend faulting on a real query) propagated
// straight out of load() as an uncaught rejection -- every one of this
// function's callers (createPersistentActor's very first await, before any of
// its own try/catch exists) crashed instead of falling back to a fresh actor.
// The outer try/catch here closes that gap for the whole function, not just
// the JSON.parse case the inner one already covered -- a caller degrades to
// "start fresh" on ANY load failure now, exactly as documented above.
export async function load(kind, key, { machineId = null } = {}) {
    try {
        ...
        log.error('snapshot load failed; starting fresh', { kind, key, err: String(e) })
        return null
    }
}
