// dockerode/tar-stream are no longer freddie dependencies (dropped to speed
// up node install) -- DockerEnvironment now always throws the "not
// available" error below until a replacement execution environment lands.
// The class/factory registration stays (src/tools/environments/index.js's
// `docker` key) so callers see a clean, unchanged error shape rather than a
// missing-export crash.
export class DockerEnvironment {
    ...
    async _ensureContainer() {
        ...
        if (!(await probeDockerode())) throw new Error('DockerEnvironment: dockerode is not installed (this environment is currently unavailable)')
