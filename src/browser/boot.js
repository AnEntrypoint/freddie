...
// ---------------------------------------------------------------------------
//
// `bootHost()` (re-exported unmodified from ../host/index.js in index.js) is
// the Node CLI host: it calls `dotenv.config()`, reads `process.cwd()`, and
// walks the real filesystem via `discoverPlugins()` to find plugin
// directories. None of that makes sense in a browser tab — there is no
// `.env` file, no process-wide cwd, and no filesystem to `readdirSync`.
// Every existing browser embedder (thebird's `docs/freddie-host.js`) already
// works around this by forking freddie's host contract wholesale into its
// own file and never calling this module's `bootHost` at all.
//
...
// It is purely ADDITIVE — bootHost/host/resetHostForTests are untouched, so
// existing Node CLI callers and any embedder still on the v1
...
import { FreddieAdapterError, required, guardStorage, guardFs, resolvePlugins } from './adapter-guards.js'

export { FreddieAdapterError }
