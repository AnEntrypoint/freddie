...
Examples:
  ...
  freddie --profile headless "run the tests"     answer one task, print the result, and exit
  freddie --profile tui --patch ./extra.yml      boot a custom profile with one extra overlay
  freddie --profile tui --resume <session>       arguments after the launcher flags reach the app
  freddie --profile web --help                   the web app's own flags and help
...
export function parseDshArgs(argv, version) {
  ...
  program
    ...
    .argument('[args...]', 'arguments for the booted profile\'s app (see: freddie --profile <name> --help)')
    .option('--profile <name>', 'the profile under $FREDDIE_HOME/profiles to boot')
  ...
  web
    ...
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the web app (see: freddie web --help)')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
